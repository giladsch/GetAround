...
const _loadOptions = async (inputValue: string): Promise<Option[]> => {
  const options = (
    await axios.post<Option[]>(`http://localhost:8000/api/places/search`, {
      value: inputValue,
    })
  ).data;
...
const getPlan = async (
  ...
  const plenPerDay = (
    await axios.post<any>(`http://localhost:8000/api/places/plan`, {
      places,
      numOfDays,
    })
  ).data;
...
const App: FC = () => {
  ...
  const numOfDays = useMemo(
    () =>
      (dates?.range1?.endDate &&
        ...
        (dates?.range1?.endDate.getTime() -
          dates?.range1?.startDate.getTime()) /
          (1000 * 60 * 60 * 24) +
          1) ||
      0,
    [dates]
  ...
  let checkInput: boolean = useMemo(
    () => places.length >= numOfDays,
    [places, numOfDays]
  );
            ...
            >
              <Fade in={dateRangeOpen}>
                <div className="date-range-field-con">
                  <DateRange
                    onChange={(newValue) =>
                      setDate((currValue) => ({ ...currValue, ...newValue }))
                    }
